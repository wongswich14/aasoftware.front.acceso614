...
const DoorsList: React.FC = () => {
    ...
    const {data: doorsData, isLoading: doorsIsLoading} = useListResidentialsDoorsQuery(id!, {skip: !id});
                ...
                <tbody>
                {
                    doors && doors.map((user, i) => (
                        <tr key={user.id}
                            ...
                            <td className='text-center whitespace-nowrap py-4 font-normal'
                                onClick={() => navigate(`/doors/${user.id}`)}>{i + 1}</td>
                            <td className='whitespace-nowrap py-4 font-normal text-left hover:underline'
                                onClick={() => navigate(`/doors/${user.id}`)}>{`${user.name}`}</td>
