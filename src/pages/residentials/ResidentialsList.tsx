...
} from "src/core/features/residentialServerApi";
...
const ResidentialsList: React.FC = () => {
    ...
    const {
        ...
        error: residentialsError,
        isLoading: residentialsIsLoading,
        refetch: refetchResidentials
    } = useListResidentialsQuery()
    const [softDelete, {
        data: softDeleteData,
        status: softDeleteStatus,
        isLoading: softDeleteIsLoading
    }] = useSoftDeleteResidentialMutation()
    const [hardDelete, {
        data: hardDeleteData,
        status: hardDeleteStatus,
        isLoading: hardDeleteIsLoading
    }] = useHardDeleteResidentialMutation()
                        ...
                        residentials && residentials.map((residential, i) => (
                            ...
                            < tr key={residential.id}
                                ...
                                <td className='whitespace-nowrap py-4 font-normal text-left'>
                                    <div className="w-fit">
                                        <Link to={`${residential.id}`} className="hover:underline w-fit">
                                ...
                                <td className='flex gap-6 items-center justify-center ml-5 py-4'>
                                    <FaEdit className='text-sky-500 hover:text-sky-400'
                                            onClick={() => toggleUpdateModal(residential.id)}/>
                                    <FaTrash className='text-red-500 hover:text-red-400'
                                             onClick={() => toggleDeleteModal(residential.id)}/>
                                </td>
                            </tr>
                        ))
