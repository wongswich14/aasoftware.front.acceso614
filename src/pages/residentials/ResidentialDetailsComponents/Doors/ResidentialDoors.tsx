...
import ResidentialUpdateDoorModal from "./ResidentialUpdateDoors.tsx";
...
const ResidentialDoors: React.FC<ResidentialInformationProps> = () => {

    const { id } = useParams<{id: string}>()
    const [doors, setDoors] = useState<DoorDto[] | null>(null)
                ...
                <tbody>
                {
                    doors && doors.map((user, i) => (
                        <tr  key={user.id} className="border-b text-gray-700 dark:border-neutral-500 hover:bg-blue-500/5 hover:cursor-pointer">
                            <td className='text-center whitespace-nowrap py-4 font-normal hover:underline'>{i + 1}</td>
                            <td className='whitespace-nowrap py-4 font-normal text-left hover:underline'>{`${user.name}`}</td>
...
export default ResidentialDoors;